...
The error increases since for a lower resolution, the velocity profile is closer to the assumed theoretical.

\begin{figure}[bp]
  \begin{minipage}[b]{0.50\textwidth}
    \centering
      \includegraphics{gfx/immersed_boundary/poiseuille_flow/2_vp/stencil.pdf}\label{fig:mask_vp}
    \par\vspace{0pt}
  \end{minipage}%
  \begin{minipage}[b]{0.50\textwidth}
    \centering%
   \begin{tabular}[b]{ccccc}\hline
      Method & Order $n$&  $\Delta_n v_x(B)$ & $\Delta_n v_x(R1) $ \\ \hline
        Vp. & 2 & 27.4991 & -0.999827\\
        Vp. & 4 & 3.05024 & 27.4991\\
        Df. & 2 &   \small{a} & -1.0\\
        Df. & 4 &   \footnote{No influence since point is set to zero.\label{note_grid_vp}}   & 31.0\\ \hline
    \end{tabular}
    \par\vspace{0pt}
  \end{minipage}
    \caption{Schematic velocity profile at the boundary point(B), in the fluid (R1, R2) and in the boundary (L1, L2).
    and Numerical evaluation of the laplacian at B and R1 for different fd-schemes.}
\label{fig:test}
\end{figure}
...
Table () compares the computation of the operator for different methods, as a velocity profile  the steady states from previously carried out simulations were used.
As we can see for the volume penalization method the second order creates a large error on the boundary however the influence is small compared to the damping force $\nu$.
For the fourth order scheme at the point R1 the laplacian is evaluated to $\approx 27.5$ which means that there is some error in this evaluation ... damn.
