...
In order to test the interpolation methods, we introduce a testcase with a curved geometry. Since for
the previous testcase the channel walls were parallel to the simulation grid no further interpolation procedures
were necessary. The simplest extension of the planar poiseuille flow is the transision to the flow through a pipe, also
called Hagen-Poiseuille flow.\\
We consider again a flow in x-Direction where the fluid domain is restricted by a wall given by the radius $r_0 = \sqrt{(x-lx/2)^2 + (y - ly/2)^2}$,
the distance from the pipe center.
With the introduction of cylindrical coordinates $(r, \phi, x)$ the equation of motion for the steady state reduces to [CITEFLUID]

\begin{align}
        0 &= - \frac{\partial p}{\partial x}  +  \frac{\nu}{r}\frac{\partial}{\partial r}\left(r\frac{\partial u}{\partial r}\right)
\end{align}

The solution is then given by integrating twice [CITEFLUID]
\begin{align}
    u &= \frac{r^2}{4\nu}\frac{\partial p}{\partial x} + A \ln r + B
\end{align}
by using the boundarie conditions $u(r = r_0) = 0$ and choose the same non-dimensionalization as in () with the lenght scale $L = r_0$,
the velocity profile for the channel is given by
\begin{align}
    u &= \frac{r^2 - r_0^2}{4}\frac{\partial p}{\partial x}Re
\end{align}
Since $v_{max} != 1$ by definition, the pressure condition for the domain is given by
\begin{align}
