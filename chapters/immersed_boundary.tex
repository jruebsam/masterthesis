...
Finally with the velocity $v(c)$ and  the No-Slip boundarie condition $v(a) = 0$,
...
The masking function given by Eq. (), for the cylinder.
...
Finally a function $\vec{g}(x,y,z )$ which defines the gradient of the immersed boundary is necessary.

\begin{align}
    \vec{g}(x, y, z) = \left(-\left(x - \frac{l_x}{2}\right),  - \left(y - \frac{l_y}{2}\right), 0\right)^T
\end{align}

A normalization of this function is performed in the algorithm, hower it is important that the normals point into direction of the fluid domain.
With these methods the algorithm does the following
