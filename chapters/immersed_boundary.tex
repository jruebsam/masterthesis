...
The cuda algorithm introduced in chapter \ref{CHAPTER:CUDA} is not yet suitable for such a scenario.
...
The common approach to extend the algorithm would be to use a body-fitted mesh ,
...
The term was first mentioned in \citep{peskin72}, where the mettod was used for the simulation of blood flow through a heartvale,
...
since the surface is often given by lagrangian points, this methods is well suited for moving boundaries \citep{Mittal2005}.
One common problem is that from a continous forcing instabilities and numerical oscillations may arise in numericial stiff problems \citep{QUOTE}.
...
\section{Immersed Bonudary Methods}
...
\begin{figure}[!b]
    ...
    \caption{Masking function $H(x,y,z=const.) = x^2 + y^2 < c$ for a cylinder. }
    \label{fig:ibm_maskvolpen}
\end{figure}

The  behind the volume penalization method is to introduce an additional forcing term into the Navier-Stokes equation, which acts on
