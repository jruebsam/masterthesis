...
Before testing the IBMs, it is reasonable to test the default algorithm on the test case, since the geometry is still not curved yet.
A grid convernce study was executed to analyze the convergence order of the methods. Finite difference stencils of second and
fourth order were used.
For the setup


\paragraph*{Setup}\mbox{}\\
For the  numerical setup the parameters were set to $l_x=1$, $l_y=1$, $l_z=1$.
The reynolds number was set to a constant value of $Re=100$ whereas the resolution was varied between $64 - 128$ with a stepsize of $N = 8$.
The timestep was set to $dt=1e-5$.
...
\begin{figure}[!bp]
    ...
    \centering
    \input{gfx/immersed_boundary/1_default/relative_l2error.pgf}
\end{figure}
