...
The most import are \textbg{global functions} also called kernel, which are called by the host and executed on the device.
...
NEUSCHREIBEN
It defines the dimension of the \textbg{grid} and the \textbg{block} of a kernel:
...
each get assigned to a different \textbg{ThreadId}.
...
In this example \textbg{dimGrid} and \textbg{dimBlock} would be defined by
\begin{verbatim}
    dim3 dimBlock(2, 2);
    dim3 dimGrid(2, 2);
    int blocksize = 2;
