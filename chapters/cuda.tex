...
Fig. () shows how this memory stucture is locatted in the numerical domain, exemplarily for $i>2$.
...
Since this terms are quite large, here this will be exemplarily shown for the viscous force
-here exemplarily for the finite diff fourth order ..
-discretization in x yz direction
-for the $v_x$ component

\begin{align}
    \Delta \vec{v_x}   \approx &  \frac{1}{12\Delta x^2} \left(-v_x(i+2, j, k) + 16v_x(i+1, j, k) - 30v_x(i, j, k) + 16v_x(i-1, j, k)-v_x(i-2, j, k)\right)\\
