...
For the temporal integration the Willamson-RK3 schemes is used.
-3 tsep hier nur der erste die anderen werdne genauso ausgeführt nur mit andaren coeffcienten.
...
West, East2, East in $x$ direction, or in short (W2, W1, E1, E2. For second order finite difference schemes,
only the inner points are used, we refer to a 3-Point stencil in this case.


\subsection{Timestep}

loop mit ix
one step in the loop can divited in three step
The computation of the actual timestep can be split into three parts.

first time:
-load 8x8xlayer in the memory
-each thread loads east east2 west west2 ant the center point
-points on the side also
-exp computation of north  bla bla

-the memroy layout in the shared memory shows in figr
-ghost points on side


ix = so undo so

this is the start array now the for loop start

-with the data now we can perfrom the computation of the discretizdet blab lba

-each veariabl is defined by theardizd
