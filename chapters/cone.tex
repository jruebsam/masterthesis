...
Inertial waves are excited by a temporal modulation of the rotation frequency.
...
The apex of the cone introduces a so to speak open geometry into the fluid domain and is acting as an attractor to inertial waves.
...
The excitation mechanism is a results from the wall friction, induced
...
the taylor-couette system showed that other velocities other than zero, could create a larger numerical error.
...
\begin{figure}[!pb]
  ...
  \caption{Kinetic energy in $z$-direction for the direct forcing second-method and different libration frequencies.
...
For an equal simulation time the error for a resonance will be in the range off $\sigma_A \approx 5\% - 10\%$.
In case of no resonance we estimate $\sigma_A \approx 2.5\% - 5\%$.\\
Secondly we want to compute the normalized helicity of the system, given by
...
Despite the the theoretical and experimental results, discussed in section \ref{cone:theorie_theo}, \ref{cone:theorie_exp},
...
A further comparison was done, by computing the eigenmodes for the possible (n, k) modes of the different peaks.
The results are shown in figure \ref{cone:cyl_modes}.
Since the pressure is not directly accessabel we computed the absolute velocity in $z$-direction, which should
correlate with the theoretical pressure gradient in $z$-direction.
Bis auf das vorzeichen sind die structuren sich sehr ähnlich.
-betrag beding sonst integral null
-eine mode fällt raus und ist nicht dargestellt.
...
\begin{figure}[!b]
  ...
  \includegraphics{gfx/cone/cylinder/numvis.pdf}  \caption{
      ...
      The red line indicates the wave vector of an inertial wave node.
...
The representation of the $v_z$-component in figure \ref{cone:cyl_modes}, in comparison to the pressure gradient
...
due to the mistakenly used aspect-ratio of $\nicefrac{5}{11}$, instead of$\nicefrac{1}{2}$.
Furthermore three additinonal modes were found in this study, which do not occur
in our frequency spectrum. due to the smaller ekman number
One explanation for this is that we use a relatively larger Ekman number used in this simulations $10^-4$.
-discussion damping breite, hidden spectrum.\\
All methods are able to reproduce these results, with exception the the interpolation method.
We were not able to figure out the reason for the numerical unstability.
The almost indentical errors of the volume penalization and direct forcing method of 2nd order,
indicate that the timestep maybe to small.
...
The numerical viscositiy was computed in order to determine the influence on the Ekman number.
For the physical properties of our system, we estimate wavelengts in the range of $1 - \Ekman^{1/3}$.
The error of the numerical viscosity in this range can be neglected, according to figure \label{cone:cyl_numvis}.

\subsubsection{Helicity}

The computation shows that the total helicity of the cylinder is zero, due to the axial symmetrie in
$z$-direction.
The helicty in the upper and lower part of the cylinder cancels each other out.
This results will be used as a comparsion to the cone.
