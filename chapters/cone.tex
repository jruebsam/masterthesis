...
For both cases, the frustum and the cone, a positive helicity can be observed in the region $\theta<\alpha$, which is of
the order $5\cdot 10^-2$.
At the critical slope $\theta=\alpha$ the helicity decreases, below zero and is of the order $-1\cdot10^{-1}$.
For $\omega>1.8$ the helicty is increasing slightly above zero to the order $10^-3$.
The differences of the helicity, between the cone and the frustum are marginal.
However it can be noted that the positions of the peaks (\RN{1]}), (\RN{2}) and (\RN{8}),
corresponds to an increase.
Overall it can be said that the computed relative helicity of the system is small in
comparsion maximum value of 1.
...
\begin{figure}[!b]
  ...
  \caption{
      ...
      Computation of the relative helicity.
    }
\end{figure}
