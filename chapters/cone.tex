...
-turbelence idee anregung und zerfall von turbelenz
...
This is an appropriate choice, since
-coordinate system
...
On the left side of the plot shows the frequency spectrum, the right side illustrates the used geometry.
The height is in descending order, from top to bottom.
Each plot contains two curves, one for the cone and the other one for the frustum.
We beginn with a qualitative description of the curves for the cone.\\
At all heights $h_+$, several resonances can be observed, for now we will consider the case where $h_+=0.25$.
For this height the largest amplitudes occur in the spectrum.
The largest mode (\RN{1}) can be observed at a frequency $\omega=1.05$, the amplitude is of the order $5\cdot10^{-4}$.
The second largest mode (\RN{2}) can be found at $\omega=0.9$ of order $3.7\cdot10^{-4}$, followed by
a possible mode (\RN{3}) at $\omega=0.5$ of order $2.8\cdot10^{-4}$.
On the right side of the spectrum there are two possible resonances at $\omega=1.5$ and $\omega=1.7$ (\RN{4}, \RN{5}).
Furthermore for the case $h_+=0.5$  we can see a resonances (\RN{6}) $\omega=0.5$. and (\RN{7}) at $\omega=1.8$.

It can be noted that all of these resonances are slightly shifted to the right
with an descending height.




The largest occurs at $\omega\approx1.05$
...
cone:
- 3 moden
-spectrum verschiebt sich nachl rechts with derceas in heihgt
- 1 large mode rechtst im intervall so und so
- 1 smaller left bis auf h=0.5 im intervall so un so
- evtl eine kleine moden links
- und 2 weitere kleine ganz kleine rechts
- ein gebiet links

frustum:
-die hauptmode ist nicht mehr vorhanden bis auf den fall h0.6
- die linke mode ist noch zu erkennen aber ist gedämpft
- für h=0.5 stimmt sie mit der mode überein von cone
-aber für größere h shiftet sie langsamer nach rechts bis sie versetzt sind
-das gebiet was link reinkommt ist ähnlich wie bei cone
...
\begin{figure}[!pt]
  ...
  \caption{
    ...
    Simulation of a cone (blue) and a frustum (green) for different offsets at the
    top.
    }
\end{figure}
