...
Despite the theoretical and experimental results, discussed in this chapter,
it is difficult to find results in literature which would be suitable for an appropriate validation.
...
It can be noted that at this position the largest resonance M\RN{1} occurs and is of the order $\approx3.5\cdot10^{-2}$.
...
For all (n, m)-modes the number of wave nodes in $z$, and $x$ direction is indentical.
...
The red bar denotes the wavevektor for the width of an inertial wave node $\propto \Ekman^{1/3.}$.
...
The width of a resconance peak is proportional to the damping rate of the system, in analogy to an harmonic oscillator.
\footnote{Privave communication A.Tilgner}
...
The error of the numerical viscosity in this range can be neglected, according to figure \ref{cone:cyl_numvis}.

\subsubsection{Helicity}

The computation shows that the total helicity of the cylinder is zero, due to the axial symmetry in $z$-direction.
