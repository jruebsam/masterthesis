...
of height $H=\SI{19.95}{\centi\meter}$ and a radius of $r=\SI{19.95}{\centi\meter}$ was used and filled with water.
...
- not adresse different reflection properties
- in this case the reflection is given by
- two regions
critical slow $\omega_c = 0$.
...
In Fig. \ref{fig:cone:cyl_vzmode}, this energy is shown exemplary for two libration frequencies for a cylinder.
...
\footnote{An uneven pressure gradient creates an even velocity field.}
...
For the simulation an Ekman number of $\Ekman =  10^{-4}$ was chosen. A simulation of higher Ekman numbers
...
\begin{align}
    T_{\text{end.}}^* = \Bigl\lfloor\left(\text{T}_{\text{end}}\frac{\omega}{2\pi}\right)\Bigr\rfloor
...
From  the order $\approx 2\cdot10^{-4}$ at $\omega=0$, to  $\approx 3.4\cdot10^{-4}$ for the cone and $\approx 4\cdot10^{-4}$ for the frustum,  at $\omega=1$.
...
Furthermore it seams that the spectrum can be divided into two domains $0\leq\omega\leq1$ and $1 \leq \omega\leq 2$.
...
to the results of the librating  cylinder, we discussed in Sec. \ref{cone:sec:lib_cylinder}.
The result for $r=0$ is identical to the simulation of the cone from the previous section.

With an decrease of the radius a change in the position and amplitude of the
...
The largest peak M\RN{1} can be observed at a frequency $\omega=1.25$, the amplitude is of the order $5\cdot10^{-4}$.
...
The amplitudes of the peaks M\RN{1} and M\RN{2} have a maximum at $h_+= 0.25$. For the peak M\RN{3} and M\RN{8} the amplitude decreases almost linear.
...
For both cases, the frustum and the cone, a positive helicity can be observed in the region $\omega<\omega_c$, which is of
the order $5\cdot 10^{-2}$.
At the critical slope $\omega=\omega_c$ the helicity decreases below zero and is of the order $10^{-1}$.
For $\omega>1.8$ the helicty is increasing slightly above zero to the order $10^{-3}$.
...
corresponds to an increase in the helicity.
...
Neither the experiment nor the theoretical description, are in accordance with the results of the simulation.
...
The propagation angle for M\RN{2} is $\theta=\arccos(0.85/2)\approx65^\circ$.
...
It has to be concerned that this are of course theoretical assumptions and a wave ray is not only emitted at the
...
Furthermore it could shown that all immersed boundary methods were able to reproduce these modes.
