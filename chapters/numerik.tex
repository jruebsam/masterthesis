...
Until now we have ignored the fact that the equations of motion i.e. Eq.  \ref{theorie:eqns}
cannot directly be solved for the pressure.
...
In order to determine the pressure it would be necessary to solve this poisson equation.
The solution is however very time consuming and difficult to implement on a GPU \citep{TILGNER}.
...
Eq. is a motified version of the continuity equation Eq. ().
With this modification the Navier-Stokes equation Eq. () is given by
