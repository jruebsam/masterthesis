...
with $b_i\in\mathbb{R}$ and $(1\leq i \leq s)$.
...
In this example the time is discretized by the explizit euler method, which is equivalent to a Runge-Kutta method of first order.
...
is small, a good estimation is $\Mach<0.1$ \footnote{Private Communication A.Tilgner}.
...
a small timestep is needed since $v=c^2$.
