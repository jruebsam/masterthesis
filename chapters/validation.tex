...
As the theoretical solution for the testcase is a polynom of 2nd order,
...
Eq. ref\label{vali:pflow_theosol}.
...
Fig. \ref{fig:example} shows exemplary the velocity profile near to the immersed boundary, for the Volume-Penalization method of 2nd and 4th order
...
In totalvelocity profiles for three different resolution are considered for a constant $J$,
the minimum of the error shall be defined as $N_min$.

Due to the negative velocity at the boundaries the first profile $N<N_min$ lies below the theoretical assumed.
The second profile is at $N=N_min$. The error from the border is smaller due to the increase in the resolution.
The proifle overlaps best with the theoretical solution resulting in the minimal error.
Finally when increasing the resolution further $N>N_min$, the error increases. The method converges towards the same resolution as the 2nd order scheme.
