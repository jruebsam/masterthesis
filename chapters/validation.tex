...
A decrease in the damping rate $J$ results in an decrease of the error about 5 orders, from $1e-0$ to $1e-5$.
With a change of Reynolds number from $500$ down to $100$ the error decrease about one order.
...
The damping in the masking area is so weak that the flow reaches the real(?) boundaries of the fluid domain,
This is not the case for $nu<1e-3$ since the velocities reaches zero before the boundaries.\\
A linear fit in the log-log space gives an error decay rate of about 1.106 for  $J$ and about $-1.223$ with $\Rey$.
...
-finally converges against a constant value.
...
(IDENTISCH ZU default method darf man das ?)
The results of the grid convergence study are shown in Fig. \ref{fig:ema1}.
The double logarithmic axis shows the dependency of the computed relative $l_2$-error norm
with respect to the grid resolution.
