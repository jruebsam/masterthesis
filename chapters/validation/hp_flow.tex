...
An additional attempt was made to interpolate the density at the border according to \citep{PAPER}
...
For a further explanation consider the change of density, for a single grid point, which is proportional to

\begin{align}
    \rho^{j+1} \propto v^j_{i+1} - v_{i-1} \propto \rho^{j-1}_{i+2} - 2 \rho^{j-1}_{i}  + \rho^{j-1}_{i-2} + \text{rem. terms.}
\end{align}

for the one-dimenisonial case and an FD2 method. The density is implicit coupled over the
velocity to itself but neighboring grid points are independent of each other.

The oscillation could be a result of an wrong computation in the density
