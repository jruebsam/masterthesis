...
the results are not presented in this thesis, since it turns out the the copmutation is also numerical unstable.

The second problem concerning this numerical implementation are the density oscillations
...
In \citep{Fadlun2000} the DF method was tested with and without the use of VF.
Furthermore a linear interpolation method, similar to the IP was used.
As a test case the formation of a vortex ring behind curvilinear noozle was tested.
The results show that for the DF method the error is worse than first order, but can be
...
All methods in this thesis have a convergence rate of at least first order.
A validation of the IP method was performed in can be found in \citep{Gilmanov2003}, \citep{Gilmanov2005}.
In \citep{Gilmanov2003} the IP method is tested by the steady flow in a lid-driven cavity containing a sphere,he error is of order $1.74$
in  \citep{Gilmanov2005} a flow caused by an oscillation sphere in a cavitiy is studied, with an error order of $1.77$.
