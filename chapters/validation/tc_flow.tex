...
\begin{figure}[!bp]
  \begin{minipage}[c]{0.5\textwidth}
      \includegraphics{gfx/immersed_boundary/tcflow/long/ts_o2.pdf}
      \caption{\label{tcflow:results_long_ts_o2}
            Averaged density for FD2 metdods with respect to the simulation time.
          }
  \end{minipage}
  \begin{minipage}[c]{0.5\textwidth}
      \includegraphics{gfx/immersed_boundary/tcflow/long/ts_o4.pdf}
      \caption{\label{tcflow:results_long_ts_o4}
...
The results of the computation are shown in Fig. for the FD2  \ref{tcflow:results_long_ts_o2}
and in Fig.  \ref{tcflow:results_long_ts_o4} for the FD4 methods.
(DESRIPTION FOLLOWS SIMULATION RESULTS)
(DESRIPTION FOLLOWS SIMULATION RESULTS)
When comparing a FD2 method to its FD4 version it can be
-the denisiy fluctuates for both DF2 and DF4.
(DESRIPTION FOLLOWS SIMULATION RESULTS)
(DESRIPTION FOLLOWS SIMULATION RESULTS)

Beside the change in the density for both cases FD2 and FD4 numerical oscillation can be
observed which  are similar to the Hagen-Poiseuille flow test case.
%OSZIS ? The profile of the other DF4 methods of is shown in Appendix \ref{fig:hpflow_allgc_theo}.
...
The by far smallest error was obtained with the IP-DF2 method.


-comparsion literaure

In the last testcase
tc flow:

- fehler wächst
- druck hat auch oszillation auch klelin
- vermutlich sind die nicht so shclimm

- the velocity profiles show that  die imb nicht gut für bewegte ränder sind
- vllt auch wegen dem presuser solver ?
