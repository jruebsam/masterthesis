...
for the VP-FD4 method about $\lambda=1.16$. However it can be seen that the convergence rate decreases above $N\approx100$,
for the VP-FD4 method the error remains approximately constant.
The optional use of the VF-method decreases the overall error, however the converges rate is about $\lambda=1.3$
for both, the FD2 and the FD4 method.

In Fig. \ref{vali:tc_flow_gc_df} the relative $l_2$-error is shown for different DF-methods.
For alle methods the error convergence rate is about first order, except the DF-FD4 method which is about $\lambda=0.9$.
...
In this validation not alle errors are decreasing linear. The
