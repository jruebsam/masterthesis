...
In comparison the Hagen-Poiseuille flow test case, in this simulation the density also changes
for the FD2 methods.
The saturation values of order $10^{-4}$ are similar  with exception of th IP+DF FD2 and DF-VF FD4 method ($10^-3$).



- divergence of v
The saturation of the density is
One assumption is that the divergence of $\vec{v}$ is not zero in th
The change in the density is proportional to the divergence of the velocity.


Hower from


One assumption was that the change in the density, which is proportional to the
diveregnce of v is




-zeitliche beschreibung der dichte
